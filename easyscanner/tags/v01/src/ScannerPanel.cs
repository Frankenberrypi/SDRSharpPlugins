...
namespace SDRSharp.Scanner
{
    ...
    public partial class ScannerPanel : UserControl
    {
        ...
        void Start()
        {
            ...
            _scanner.ThresholdReleaseTime = 10000;
        ...
        void ScanNextFrequencyTimeLeftCallback(double Seconds, ScannerState State)
        {
            ...
            try
            {
                ...
                if (_scanner.State == ScannerState.StayTuned)
                {
                    ...
                    if (_scanner.IsStayTunedStable)
                        textBoxTimeout.Text = "disable";
                    else
                      textBoxTimeout.Text = String.Format("{0:0.0}", Seconds);
        ...
        void ScanNextFrequencyCallback(int MemoryIndex)
        {
            ...
            foreach (DataGridViewRow row in frequencyDataGridView.Rows)
            {
                if (i++ == MemoryIndex)
                {
                    ...
                    if (MemoryIndex > 5 && MemoryIndex % 4 == 0)
                    {
                        frequencyDataGridView.FirstDisplayedScrollingRowIndex = MemoryIndex - 1;
                    }
                }
            }
